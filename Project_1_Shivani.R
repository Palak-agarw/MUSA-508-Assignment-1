...
#graduated buffer
ggplot()+
  geom_sf(data = boroughs, fill = '#f0f0f0', 
          color = 'white')+
  geom_point(data= ntaCensus %>%
               filter(totalVenues > 0), 
             aes(x=lon, y=lat, size = totalVenues), 
             color=palette[3], 
             alpha = 0.6) + 
  scale_size_area(name="", max_size = 9) + 
  guides(size=guide_legend("Total Venues")) +
  geom_text(data=boroughs, aes(x=lon, y=lat, label=boro_name), alpha = 0.65)+
  labs(
