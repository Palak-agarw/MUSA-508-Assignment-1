...
Now that we have found the tracts whose centroids that fall within the buffer zone, lets classify those tracts as **TOD** - transit oriented developemnt and **Non-TOD**.
...
Now lets visualise rent as a function of time and space.
...
As it is evident in the map, most tracts in San Mateo, San Francisco, Contra Costa and Santa Clara saw an increase in rent from year 2009 to 2017 while Alameda remained stagnant. Other interesting observation is that tracts close to the transit line saw a large increase in rent values. The transit stops are connected through bus and biking networks as extra infrastructure, we could have affected the other tracts to develop as well.

### Percentage of Married people

Our hypothesis is that a lot of families move out of the city to the suburbs to have more space and raise families. But still prefer to be able have easy access to the city as it forms a major job hub and center. So they would prefer to stay around the transit to avoid evryday driving and parking hassle. 
...
ggplot(finalTract.group)+
  ...
  labs(title = "Percentage of White 2009-2017", subtitle = "Bay area, CA", caption="Figure 3.3") +
  facet_wrap(~year + TOD)+
  mapTheme() + 
  theme(plot.title = element_text(size=22))
```

From the plots, we can see that the percentage of white people near the transit have reduced which would mean that it is filled by other racial groups. That is the over all trend even outside the buffer zone. Many factors can have caused this change in racial composition, but one important one is job creation and the groups those jobs attract. 

### Percentage of working class

As mentioned already, BARt connects major job centers. So its only fair to hypothesize that working class would prefer to find housing in and around transit for easy commuting. For this map, we considered age group from 16-54 as the population most likely to form the working class. 
...
There are very few  tracts which have seen a change from 2009 and 2017 to make any solid conclusions. There is no visible trend even in the Non-TOD tracts hence we cannot infer much from this map.
...
To do this first we found each station and the tracts it intersects with. After that we summed the normalized population and rent values to find the population and rent quantities. We felt that using the normalised data is a better representation because as mentioned before we dont know the population or rent distribution in each tract to know if it lies in or out of the 0.5 mile buffer.
...
ggplot()+
  ...
  geom_sf(data=popgrad, 
          ...
          color = "red")+
  scale_size_area(name="", max_size = 3.0) + 
